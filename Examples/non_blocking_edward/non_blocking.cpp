...
{   
  ...
  */
  ...
  {   
    ...
    /*The requeset was accepted therefor we send the END REQ to the memory*/
...
struct Router: sc_module
{
  ...
  {   
    ...
    if (phase == tlm::END_RESP) {  
      ...
      phase = tlm::END_RESP; 
      status = targetSocket->nb_transport_bw( trans, phase, delay );
      wait(delay);
  ...
  {   
    while (true) {
      ...
      case tlm::TLM_ACCEPTED:   
        ...
        cout << name() << " END_REQ SENT" << " TRANS ID " << id_extension->transaction_id << " at time " << sc_time_stamp() << endl;
        // Expect response on the backward path  
        phase_pending = tlm::END_REQ; 
        status = initSocket->nb_transport_fw(*trans_pending, phase_pending, delay_pending);  // Non-blocking transport call
        break;   
      ...
      case tlm::TLM_COMPLETED:   
        ...
        // Initiator obliged to check response status   
        if (trans_pending->is_response_error() )   
          SC_REPORT_ERROR("TLM2", "Response error from nb_transport_fw");   
         cout<<phase_pending<<"->TERMINE thread_Router\n";
        //cout << "trans/fw = { " << (trans_pending->cmd ? 'W' : 'R') << ", " << hex << i << " } , data = "   
